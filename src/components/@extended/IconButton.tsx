...
function getColorStyle({ variant, theme, color }: GetColorStyleProps) {
...
const IconButtonStyle = styled(MuiIconButton, { shouldForwardProp: (prop) => prop !== 'variant' && prop !== 'shape' })(
  ({ theme, variant, shape, color }: IconButtonProps) => ({
    position: 'relative',
    '::after': {
      content: '""',
      display: 'block',
...
const IconButton = forwardRef(({ variant = 'text', shape = 'square', children, color = 'primary', ...others }: any, ref: any) => {
