...
export const strengthColor = (count: number) => {
  if (count < 2) return { label: 'Buruk', color: 'error.main' };
  if (count < 3) return { label: 'Lemah', color: 'warning.main' };
  if (count < 4) return { label: 'Normal', color: 'warning.dark' };
  if (count < 5) return { label: 'Bagus', color: 'success.main' };
  if (count < 6) return { label: 'Kuat', color: 'success.dark' };
  return { label: 'Buruk', color: 'error.main' };
};
